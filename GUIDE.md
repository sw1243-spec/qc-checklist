...
| 🟩 **개발(진세운)** | `app/`, `lib/`, `prisma/` | 화면·로직·DB 구조 수정 |
| ⬜ **아무도 안 만짐** | `node_modules/`, `.next/`, 자동생성 파일 | 빌드가 알아서 생성 |
| 🟨 **운영 데이터** | `storage/`, SQL Server DB | 백업 대상. 덮어쓰기 금지 |

**핵심:** IT선배님들이 수정 필요한 곳은 `.env` 한 곳 + 빌드 명령만. DB 테이블 매핑은 Prisma가 자동 → 손으로 매핑할 게 없음.
...
⚠️ 1~2는 안전, 3은 React 코드라 신중히 작업해야함 IT 팀이 여러 작업이 필요하다고 해도 이 작업은 AI에게 맡기는 것을 추천 
...
## 6. 데이터는 어디 있나 (백업 대상) - 템플릿만 현재 백업대상으로 둠 개별작업하기 어려움 1회 서버에서 실행시 그 다음부터는 db 연동작동
...
#혹시 슬랙 안될시에 슬랙 앱에서 Incoming WebHooks. send data into Slack in real-time.  이거 깔아서 나오는 거 어드민 페이지에서 연결. 지금 회사 slack은 앱 개수 제한 10개를 다 사용중. 필요없는 것 삭제하고 추가하면 바로 사용가능.
